...
const formatModuleName = (moduleId: string | undefined | null): string => {
  ...
  const modules: Record<string, string> = {
    ...
    AUDIT: 'Auditoría', AUTH: 'Autenticación',
  };
...
export default function AuditTable({ data, loading, total, page, limit, onPageChange, onRowsPerPageChange }: Props) {
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <TableContainer component={Paper} sx={{ borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', border: '1px solid #f1f5f9' }}>
        <Table size="small">
          <TableHead>
            <TableRow sx={{ bgcolor: '#080769' }}>
              {['Fecha / Hora', 'Usuario', 'Acción', 'Módulo', 'IP', 'Detalles'].map((h) => (
                <TableCell key={h} sx={{ fontWeight: 700, color: '#fff', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px', py: 1.5 }}>
                  {h}
                </TableCell>
              ))}
            </TableRow>
          ...
          <TableBody>
            {data.map((log) => {
              const config = ACTION_CONFIG[log.action] || { label: log.action, color: 'default' as const, icon: HistoryIcon };
              ...
              return (
                <TableRow key={log._id || Math.random()} hover onClick={() => handleRowClick(log)} sx={{ cursor: 'pointer', '&:hover': { bgcolor: '#f8fafc' }, transition: 'background-color 0.15s ease' }}>
                  <TableCell>
                    {eventDate ? (
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.85rem' }}>{eventDate.format('DD/MM/YYYY')}</Typography>
                        <Typography variant="caption" sx={{ color: '#64748b', fontSize: '0.75rem' }}>{eventDate.format('HH:mm:ss')}</Typography>
                      </Box>
                    ) : <Typography variant="body2" sx={{ color: '#94a3b8' }}>—</Typography>}
                  </TableCell>
                  <TableCell><Typography variant="body2" sx={{ fontSize: '0.85rem' }}>{log.userEmail || '—'}</Typography></TableCell>
                  <TableCell>
                    <Chip icon={<IconComponent sx={{ fontSize: 16 }} />} label={config.label} size="small" color={config.color} sx={{ fontWeight: 600, fontSize: '0.72rem' }} />
                  </TableCell>
                  <TableCell>
                    {moduleName !== '—' ? (
                      <Chip label={moduleName} size="small" variant="outlined" sx={{ fontWeight: 600, fontSize: '0.72rem', borderColor: '#e2e8f0' }} />
                    ) : <Typography variant="body2" sx={{ color: '#94a3b8' }}>—</Typography>}
                  </TableCell>
                  <TableCell>
                    <Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: '0.8rem', color: '#475569', bgcolor: '#f1f5f9', px: 1, py: 0.3, borderRadius: '4px', display: 'inline-block' }}>
                      {log.ipAddress || '—'}
                    </Typography>
                  </TableCell>
                  <TableCell>
      ...
      <AuditDetailModal open={!!selectedLog} onClose={handleCloseModal} log={selectedLog} />
    </Box>
  );
}