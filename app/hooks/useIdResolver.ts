// app/hooks/useIdResolver.ts (versión actualizada)
"use client";
...
const loadLookupMap = (): Promise<Map<string, string>> => {
  ...
  if (!sharedPromise) {
    sharedPromise = buildLookupMap()
      .then((map) => { sharedMap = map; return map; })
      .catch((err) => { sharedPromise = null; throw err; });
  }
...
export const useIdResolver = () => {
  ...
  useEffect(() => {
    ...
    loadLookupMap()
      .then((map) => { if (isMounted) { setLookupMap(map); setReady(true); } })
      .catch((error) => {
  ...
  // ✅ NUEVO: reemplaza IDs dentro de texto libre ("DELETE en SERVICE (ID: xxx)")
