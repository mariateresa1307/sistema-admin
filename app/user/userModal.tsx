...
import {
  Dialog, DialogContent, DialogActions, IconButton, Typography, Button, TextField, LinearProgress, Alert,
  Box, FormControl, InputLabel, OutlinedInput, InputAdornment, styled, Divider, Switch, Chip, MenuItem,
} from "@mui/material";
import Grid from "@mui/material/Grid";
import { Close as CloseIcon, Save as SaveIcon, Visibility, VisibilityOff, AccountCircle as AccountCircleIcon } from "@mui/icons-material";
...
const CustomStyledDialog = styled(Dialog)(({ theme }) => ({
  ...
  "& .MuiPaper-root": {
    ...
    boxShadow:
      "0px 10px 40px rgba(0,0,0,0.06), 0px 20px 70px rgba(0,0,20,0.04)",
...
const ROLES_USUARIO = [
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Luego llamar a onSubmit (que en el padre mostrará la notificación)
      await onSubmit();

    } catch (err: any) {
      setError(err.response?.data?.message || "Error interno del servidor (500)");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Box component="form" ref={formRef} onSubmit={handleSubmit}>
        ...
        {loading && (
          <LinearProgress
            sx={{ mx: 4, mt: 1, height: "3px", borderRadius: "2px" }}
          />
        )}
